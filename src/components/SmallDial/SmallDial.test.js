...
test('the angle passed as a prop is used to set the style prop on the dial node', () => {
    ...
    expect(wrapper.find('.dial').first().prop('style')).toEqual({ transform: 'rotate(-83deg)' });
});
...
test(`the functions for starting, updating and ending interactions with the dial are fired 
    ...
    // test mouse events
    wrapper.find('.dial__container').first().simulate('mouseDown');
    expect(mockedStartInteraction).toHaveBeenCalledTimes(1);
    wrapper.find('.dial__container').first().simulate('mouseMove', { clientX: 50, clientY: 30, persist: () => {} });
    ...
    wrapper.find('.dial__container').first().simulate('mouseUp');
    expect(mockedEndInteraction).toHaveBeenCalledTimes(1);
    // test touch events
    wrapper.find('.dial__container').first().simulate('touchStart');
    expect(mockedStartInteraction).toHaveBeenCalledTimes(2);
    wrapper.find('.dial__container').first().simulate('touchMove', { 
        clientX: 60, 
        clientY: 40, 
        persist: () => {},
        preventDefault: () => {} 
    ...
    wrapper.find('.dial__container').first().simulate('touchEnd');
...
test('handleKeyDown and handleClick functions are fired when their corresponding events occur', () => {
    ...
    wrapper.find('.dial__container').first().simulate('keyDown', { key: 'A' });
    expect(mockedHandleKeyDown).toHaveBeenCalledTimes(1);
    expect(mockedHandleKeyDown.mock.calls[0][0]).toEqual({ key: 'A' });
    wrapper.find('.dial__container').first().simulate('click');
...
test('updateValueCallback function gets called correctly', () => {
    ...
    wrapper.find('.dial__number-input').first().simulate('change', { target: { value: '45' } });
